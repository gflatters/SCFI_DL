...
def fit_model_from_labels(train_labels,video_path,model,sample_gap,epochs,stepsperepoch,params_train,params_validate):
    ...
    #G - removed train_index
    cells_test = np.array(train_labels)[:,0]
    labels_test= np.array(train_labels)[:,1].astype('int')
    im_paths_test, im_labels_test = sample_label_images_in_cells(cells_test,video_path,labels_test,sample_gap)
    im_labels_test = dict(zip(im_paths_test,im_labels_test))
    #
...
def k_fold_train(train_labels,video_path,model,sample_gap,epochs,stepsperepoch,params_train,params_validate):
...
def k_fold_train_3d2(train_labels,video_path,model,sample_gap,epochs,stepsperepoch,params_train,params_validate,depth,sequence_length):
